...
function generatePermutedAlphabet(keyword) {
  ...
  const seen = new Set();
  const processedKeyword = keyword.toUpperCase().replace(/[^A-Z]/g, "");

  for (let char of processedKeyword) {
    if (!seen.has(char)) {
      permutedAlphabet += char;
      seen.add(char);
    }
...
module.exports = {
  caesarCipherWithKeyword,
  generatePermutedAlphabet,
  encryptWithPermutedAlphabet,
  decryptWithPermutedAlphabet,
};
